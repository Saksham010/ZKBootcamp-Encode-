...
//#########################################################################################
@storage_var
func whitelist(user:felt) -> (allow: felt) {
}
...
func transfer{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    //Checking if the amount is even or not
...
func faucet{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(amount: Uint256) -> (
    ...
    let (bool) = uint256_le(amount,Uint256(10000,0));
    with_attr error_message("Only 10,000 token can be minted at a time"){
        assert bool = 1;
    }
...
func burn{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(amount: Uint256) -> (
    ...
    // Taking 10% of the amount
    let (res,r) = uint256_unsigned_div_rem(amount,Uint256(10,0));
    let (burn_amount) = uint256_sub(amount,res);

    // Sending deployer 10%
    let (deployer) = get_admin();
    ERC20_transfer(deployer,res);
...
func exclusive_faucet{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    //Assert whether or not the caller has been whitelisted or not
    with_attr error_message("The caller has not been whitelisted"){
        assert allowed = 1;
    }
